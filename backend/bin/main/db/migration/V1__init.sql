...
-- Source of truth: docs/02-database-schema.md (§3 tables, §4 enums, §7 indexes, §8 order).
-- Forward-only / expand-contract. Enums are varchar + CHECK; PKs are uuid via pgcrypto.
...
CREATE TABLE event_assignment (
    ...
    created_at  timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_event_assignment_event_user UNIQUE (event_id, user_id)
);
...
CREATE INDEX idx_material_history_material_created ON material_status_history (material_id, created_at);
...
-- ── 3.9 registration_form (dynamic form definition, JSONB; 1:1 with event) ───
...
-- ── 3.11 event_checkin (organizer-confirmed attendance; idempotent) ──────────
...
CREATE INDEX idx_checkin_event_checked_in ON event_checkin (event_id, checked_in_at);
CREATE INDEX idx_checkin_submission ON event_checkin (submission_id);
