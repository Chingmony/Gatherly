/** Per-event public presentation (category + gradient + blurb) keyed by id. */
export const EVENT_PRESENTATION: Record<number, { cat: string; grad: string; blurb: string }> = {
  1: {
    cat: 'Conference',
...
export function presentationFor(id: number) {
  return EVENT_PRESENTATION[id] ?? EVENT_PRESENTATION[1]
}
